//! The Launcher application backend.
//!
...
//! here is only what is specific to Launcher, which is what keeps the three
...
pub fn run() {
    ...
    let state = slate_runtime::bootstrap(KnownApp::Launcher.id())
