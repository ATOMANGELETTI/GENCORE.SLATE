//! The Explorer application backend.
//!
...
//! here is only what is specific to Explorer, which is what keeps the three
...
pub fn run() {
    ...
    let state = slate_runtime::bootstrap(KnownApp::Explorer.id())
