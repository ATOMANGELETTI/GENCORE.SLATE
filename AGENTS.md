...
| Rule | Covers | Applies |
| --- | --- | --- |
| [`00-overview`](.agents/rules/00-overview.md) | What GENCORE.SLATE is, how it is laid out, and the invariants that must never be broken. | always |
| [`01-tooling`](.agents/rules/01-tooling.md) | The toolchain. Bun is the only JavaScript runtime and package manager; Moon orchestrates; Cargo builds Rust. | always |
| [`02-monorepo`](.agents/rules/02-monorepo.md) | How the workspace is organised, how projects depend on each other, and how tasks are defined. | on match |
| [`03-naming-and-structure`](.agents/rules/03-naming-and-structure.md) | The modular folder and file naming system used across TypeScript and Rust. Read this before creating any file. | always |
| [`04-typescript`](.agents/rules/04-typescript.md) | Type-safety rules, module conventions, and the patterns expected in every .ts and .tsx file. | on match |
| [`05-react-and-ui`](.agents/rules/05-react-and-ui.md) | Component structure, state management, and how application code consumes the shared UI kit. | on match |
| [`06-design-system`](.agents/rules/06-design-system.md) | The visual language — tokens, theming, and the macOS-inspired rules every surface follows. | on match |
| [`07-rust`](.agents/rules/07-rust.md) | Conventions for every crate — error handling, module layout, dependencies, and the lints that are denied. | on match |
| [`08-tauri-and-security`](.agents/rules/08-tauri-and-security.md) | Capabilities, content security policy, IPC command design, and the project's known security limitations. | on match |
| [`09-portability`](.agents/rules/09-portability.md) | The rules that keep the suite fully self-contained. This is the product's defining constraint. | always |
| [`10-testing`](.agents/rules/10-testing.md) | Where tests live, what they cover, and the crate-design consequence of keeping Rust tests out of source files. | always |
| [`11-git-and-commits`](.agents/rules/11-git-and-commits.md) | Branch naming, Conventional Commits with an enforced scope list, and what a reviewable pull request looks like. | on match |
| [`12-ci-and-release`](.agents/rules/12-ci-and-release.md) | What runs in continuous integration, how the single suite version is propagated, and how a release is cut. | on match |
| [`13-agent-workflow`](.agents/rules/13-agent-workflow.md) | How an AI assistant should approach work in this repository — what to read first, what to verify, and what to never do. | always |
...
- **Monorepo and Moon** — How the workspace is organised, how projects depend on each other, and how tasks are defined.
  `.agents/rules/02-monorepo.md` (**/moon.yml, .moon/**/*, **/package.json, Cargo.toml)
- **TypeScript** — Type-safety rules, module conventions, and the patterns expected in every .ts and .tsx file.
  `.agents/rules/04-typescript.md` (**/*.ts, **/*.tsx)
- **React and application UI** — Component structure, state management, and how application code consumes the shared UI kit.
  `.agents/rules/05-react-and-ui.md` (**/*.tsx, packages/slate-ui-kit/**/*, tauri/*/src/**/*)
- **Design system** — The visual language — tokens, theming, and the macOS-inspired rules every surface follows.
  `.agents/rules/06-design-system.md` (packages/slate-tokens/**/*, packages/slate-ui-kit/**/*, **/*.css, **/*.tsx)
- **Rust** — Conventions for every crate — error handling, module layout, dependencies, and the lints that are denied.
  `.agents/rules/07-rust.md` (crates/**/*.rs, tauri/*/src-tauri/**/*.rs, **/Cargo.toml)
- **Tauri and security** — Capabilities, content security policy, IPC command design, and the project's known security limitations.
  `.agents/rules/08-tauri-and-security.md` (tauri/**/*, crates/slate-runtime/**/*, crates/slate-ipc/**/*)
- **Git, commits, and pull requests** — Branch naming, Conventional Commits with an enforced scope list, and what a reviewable pull request looks like.
  `.agents/rules/11-git-and-commits.md` (**/*)
- **CI and release** — What runs in continuous integration, how the single suite version is propagated, and how a release is cut.
  `.agents/rules/12-ci-and-release.md` (.github/**/*, scripts/**/*, .changeset/**/*)
